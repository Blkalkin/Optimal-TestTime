...
async def get_questions(num_examples=100):
    ...
    if 0 < num_examples < len(questions):
...
async def main_async():
    ...
    # Load questions
    questions = await get_questions(num_examples=args.num_examples)
    if not questions:
    ...
    claude_file = await generate_claude_answers(
        ...
        output_file=f"simpleqa_{args.claude_model.replace('-', '_')}_responses.jsonl"
    )
    
    openai_file = await generate_openai_answers(
        questions, 
        model=args.openai_model,
        output_file=f"simpleqa_{args.openai_model.replace('-', '_')}_responses.jsonl"
    )
